...
router.post("/Procedure/adddataImages", upload.single('photo'), async (req, res) => {
  try {
    const { pname, description, price, doctor_id, duration, doctor_name, providerId, speciality, section } = req.body;
    const file = req.file;
    if (!file) {
    ...
    conn.query(
      "INSERT INTO procedures (pname, description, doctor_id, price, duration, doctor_name, provider_id, speciality, section, procedure_image) VALUES (?,?,?,?,?,?,?,?,?,?)",
      [pname, description, doctor_id, price, duration, doctor_name, providerId, speciality, section, imagePath],
      (error, results) => {
        console.log(results);
        if (error) {
          console.error(error);
...
router.put("/Procedure/update/:id", (req, res) => {
  try {
    const { pname, description, price, doctor_id, duration, doctor_name, providerId, speciality, section } = req.body;
    // const file = req.file;
    ...
    //     const imagePath = image_url.toString();
        ...
        conn.query(
          "UPDATE procedures SET pname=?, description=?, price=?, doctor_id=?, duration=?, doctor_name=?, provider_id=?, speciality=?, section=? WHERE id=?",
          [pname, description, price, doctor_id, duration, doctor_name, providerId, speciality, section, procedureId],
          (error, results) => {
            if (error) {
              console.error(error);
